...
namespace Artemis.Plugins.Wrappers.Logitech.Prerequisites
{
    internal class LogitechWrapperPrerequisite : PluginPrerequisite
    {
        ...
        public override bool IsMet()
        {
            ...
            bool is64BitKeyPresent = key64?.GetValue(null)?.ToString() == _dllPath64;
            bool is32BitKeyPresent = key32?.GetValue(null)?.ToString() == _dllPath32;
        ...
        public LogitechWrapperPrerequisite(Plugin plugin)
        {
            ...
            {
                new RunPowerShellAction("Patch 64 bit registry", patchScript, true, $"\"HKLM:{REGISTRY_PATH_64}\" \"{_dllPath64}\""),
                new RunPowerShellAction("Patch 32 bit registry", patchScript, true, $"\"HKLM:{REGISTRY_PATH_32}\" \"{_dllPath32}\"")
            };
